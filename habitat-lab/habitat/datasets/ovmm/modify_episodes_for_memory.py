...
def get_rec_category(rec_id, rec_category_mapping=None):
    ...
    # check if rec_id has "part_" in it, if so, remove it
...
def get_obj_rec_cat_in_eps(
    ...
    """
    obj_cats, start_rec_cats, goal_rec_cats = [], [], []
    for idx in range(len(episode["target_receptacles"])):
        obj_cats.append(get_obj_category(
            list(episode["info"]["object_labels"].keys())[idx].split(":")[0][:-1],
            obj_category_mapping=obj_category_mapping,
        ))

        start_rec_cat = get_rec_category(
            episode["target_receptacles"][idx][0].split(":")[0],
            rec_category_mapping=rec_category_mapping,
        )
        assert start_rec_cat is not None, f"Start receptacle category not found for {episode['target_receptacles'][idx][0]}"
        start_rec_cats.append(start_rec_cat)
        
        goal_rec_cat = get_rec_category(
            episode["goal_receptacles"][idx][0].split(":")[0],
            rec_category_mapping=rec_category_mapping,
        )
        assert goal_rec_cat is not None, f"Goal receptacle category not found for {episode['goal_receptacles'][idx][0]}"
        goal_rec_cats.append(goal_rec_cat)
...
def initialize_sim(
    ...
    sensors = {
        "semantic": {
            ...
            "resolution": config.agent_camera_resolution,
            "position": [0, 0, 0],
            "orientation": [config.agent_camera_tilt_degrees * np.pi / 180, 0, 0.0],
            "hfov": config.agent_camera_hfov,
        },
    }
    if debug_viz:
        sensors["color"] = {
            ...
            "position": [0, 0, 0],
            "orientation": [config.agent_camera_tilt_degrees * np.pi / 180, 0, 0.0],
            "hfov": config.agent_camera_.hfov,
        }
    sensor_specs = []
    for sensor_uuid, sensor_params in sensors.items():
        ...
        sensor_spec.position = mn.Vector3(sensor_params["position"])
        sensor_spec.orientation = mn.Vector3(sensor_params["orientation"])
...
def get_candidate_starts(
    ...
    for i, (obj, pos) in enumerate(objects):
        current_obj_category = get_obj_category(
            obj.split(".")[0], obj_category_mapping=obj_category_mapping
        )
        if current_obj_category in categories:
            obj_goal = {
                "position": np.array(pos)[:3, 3].tolist(),
                "object_name": obj,
                "object_id": str(i),
                "object_category": current_obj_category,
                "view_points": [],
            }
            obj_goals_hard.append(obj_goal)
            # use parent object name if it exists
            rec_name = name_to_receptacle[obj_idx_to_name[i]]
            target_recep_name = rec_to_parent_obj[rec_name]
            # otherwise use receptacle name
            if target_recep_name is None:
                target_recep_name = rec_name
            recep_cat = get_rec_category(
                target_recep_name.split(":")[0],
                rec_category_mapping=rec_category_mapping,
            )
            if recep_cat in start_rec_cats:
                obj_goals_easy.append(obj_goal)
    return obj_goals_easy, obj_goals_hard


def get_candidate_receptacles(recep_goals, goal_recep_categories):
    goal_receptacles_of_category = [
        g for g in recep_goals if g["object_category"] in goal_recep_categories
    ]
    if len(goal_receptacles_of_category) == 0:
        raise ValueError("No goal receptacles found for category")
    return goal_receptacles_of_category
...
def add_cat_fields_to_episodes(
    ...
    for episode in episodes["episodes"]:
        ...
        obj_cats, start_rec_cats, goal_rec_cats = get_obj_rec_cat_in_eps(
            episode,
            obj_category_mapping=obj_category_mapping,
            rec_category_mapping=rec_category_mapping,
        )

        if start_rec_cats == goal_rec_cats:
            # TODO (Karmesh): Handle case for multiple receptacles
            print("Start and goal receptacle category are same")
            print("Skipping episode", episode["episode_id"])
            continue

        episode["object_category"] = obj_cats
        episode["start_recep_category"] = start_rec_cats
        episode["goal_recep_category"] = goal_rec_cats
        try:
            (
            ...
            print("Skipping episode", episode["episode_id"] + " due to KeyError")
            continue

        episode["candidate_start_receps"] = get_candidate_receptacles(
            all_rec_goals, start_rec_cats
        )
        episode["candidate_goal_receps"] = get_candidate_receptacles(
            all_rec_goals, goal_rec_cats
        )
        if enable_add_viewpoints and not add_viewpoints(
            ...
            rec_viewpoints,
            rec_to_parent_obj,
            config.agent_camera_height,
            debug_viz,
        ):
            print("Skipping episode", episode["episode_id"], "because, no viewpoints found")
            continue
        assert (
            len(episode["candidate_objects"]) > 0
            and len(episode["candidate_start_receps"]) > 0
            and len(episode["candidate_goal_receps"]) > 0
        ), f"Episode {episode['episode_id']} has no candidate objects or receptacles"
